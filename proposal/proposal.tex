...
\midrule
\href{https://exercisedb.dev/}{ExerciseDB (raw flat)} (ExerciseDB, n.d.) & 1{,}500 & Exercise catalogue: body parts, equipment, target muscles, step-by-step instructions. Used by the exercise recommender and embedded for RAG. \\
\href{https://www.kaggle.com/datasets/valakhorasani/gym-members-exercise-dataset}{Gym Members Exercise Tracking} (Khorasani, 2024) & 973 & Real workout sessions with calories burned, BPM (max/avg/resting), session duration, workout type, BMI, and experience level. Primary training data for the predictive models. \\
\href{https://www.kaggle.com/datasets/uom190346a/sleep-health-and-lifestyle-dataset}{Sleep Health \& Lifestyle} (Tharmalingam, 2023) & 374 & Provides clusters for the descriptive lifestyle-profile view and supplies the rule-based \emph{Training Readiness} auxiliary feature. \\
\href{https://www.kaggle.com/datasets/adilshamim8/daily-food-and-nutrition-dataset}{Daily Food Nutrition} (Shamim, 2025) & 651 & Provides macro/calorie context shown on the dashboard and used in nutrition-aware rule snippets retrieved by RAG. \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{itemize}
  \item \textbf{Calorie-burn-rate regression} (kcal\,/\,hr; session duration excluded so the model learns physiology rather than the trivial duration--calories relationship, $r{=}0.91$): \emph{Ridge Regression}, \emph{Random Forest Regressor}, \emph{XGBoost Regressor} (Chen \& Guestrin, 2016). Features: demographics, body composition, BPM, fat percentage, workout type, experience, water intake. Metrics: RMSE, MAE, $R^2$.
  \item \textbf{Workout-intensity classification} (3-class Low / Mid / High, derived from the average-to-maximum BPM ratio binned by tertiles $\le 0.70$, $0.70$--$0.85$, $\ge 0.85$): \emph{Multinomial Logistic Regression}, \emph{Random Forest Classifier}, \emph{XGBoost Classifier}. Features: demographics, body composition, resting BPM, fat percentage, experience level, auxiliary readiness. Metrics: accuracy, macro-F1, confusion matrix.
\end{itemize}
...
\begin{list}{}{\setlength{\leftmargin}{1.5em}\setlength{\itemindent}{-1.5em}\setlength{\itemsep}{3pt}\setlength{\parsep}{0pt}\setlength{\topsep}{0pt}}
...
\item Lewis, P., Perez, E., Piktus, A., Petroni, F., Karpukhin, V., Goyal, N., K\"uttler, H., Lewis, M., Yih, W., Rockt\"aschel, T., Riedel, S., \& Kiela, D. (2020). Retrieval-augmented generation for knowledge-intensive NLP tasks. \emph{Advances in Neural Information Processing Systems, 33}, 9459--9474.
