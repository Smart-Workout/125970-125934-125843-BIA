...
\begin{itemize}
  ...
  \item \textbf{Predictive analytics:} train and compare multiple candidate models per task via cross-validation: (a) \emph{calorie-burn-rate} regression (kcal\,/\,hr); (b) \emph{Workout-intensity} 3-class classification (Low/Medium/High).
...
Eight source tables/files are used across the system, organized into five public dataset groups. Four of these sources come from the Gym Database Management System and form a relational star-style structure: users, check-in/check-out history, subscription plans, and gym locations linked by shared keys. The remaining four standalone datasets support ML training, exercise retrieval, lifestyle clustering, and nutrition context.
...
\begin{itemize}
  ...
  \item \textbf{Workout-intensity classification} (3-class Low/Medium/High, derived from BPM indicators; label-derived features excluded from inputs): \emph{Multinomial Logistic Regression}, \emph{Random Forest Classifier}, \emph{XGBoost Classifier}. Metrics: accuracy, macro-F1, confusion matrix.
\end{itemize}
