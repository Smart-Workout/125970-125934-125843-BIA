...
Smart Workout is a Retrieval-Augmented Generation (RAG)-assisted Decision Support System and Business Intelligence dashboard for personalized weight-training planning. The project integrates gym membership and check-in records, exercise-tracking data, lifestyle and sleep indicators, nutrition context, and an exercise knowledge base into one descriptive, predictive, and prescriptive analytics workflow. The implemented prototype uses a React frontend, FastAPI backend, XGBoost predictive models, rule-based readiness logic, RAG-supported exercise retrieval, and an embedded Tableau map.

The system supports two decision contexts. For executive users, it summarizes branch utilization, time-based demand, subscription and estimated revenue signals, lifestyle segments, and operational action priorities. For individual users, it converts profile, wellness, goal, equipment, and constraint inputs into readiness-aware workout guidance. Feature engineering produces both numerical outputs, including readiness score, estimated calorie-burn rate, session calorie target, and final training score, and categorical outputs, including readiness band, predicted Low/Mid/High intensity class, priority action, and plan rationale category.

Two supervised learning tasks were evaluated with 5-fold cross-validation, and XGBoost was selected as the final model for both. The XGBoost Regressor achieved RMSE 15.021 kcal/hr and $R^2=0.970$ for calorie-burn-rate estimation; this value is reported as a deployment-aware upper bound because average BPM is estimated at inference. The XGBoost Classifier achieved macro-F1 0.342 for three-class workout-intensity prediction. Macro-F1 is used instead of accuracy because the classes are imbalanced and the model remains below the majority-class baseline on raw accuracy. The final DSS/BIS output is an explainable dashboard and plan-generation workflow that links engineered data, transparent model results, RAG-grounded recommendations, and business decision mapping.
...
\end{equation}
The 0.60/0.40 weighting is a design heuristic that prioritizes recovery readiness over predicted intensity; it should be recalibrated with real adherence and outcome data in future work. A readiness cap is applied when recovery is low so that the prescribed volume cannot exceed the user's current recovery condition, even if the predicted intensity class is high.
...
Numerical outputs & Calorie-burn-rate estimate, session calorie target, readiness score, final training score, sets, reps, rest seconds, exercise count & Reported as measurable plan and dashboard values that support calorie targeting, workload control, and weekly progress monitoring. \\
...
\begin{figure}[H]
    ...
    \caption{Backend implementation of the Smart Workout prototype.}
...
Random Forest Regressor & 15.697 & 10.834 & 0.967 & No \\
...
\centering
\small
\caption{Cross-validated classifier comparison for three-class workout-intensity prediction, including trivial baselines. XGBoost is selected on macro-F1; no model exceeds the majority baseline on raw accuracy, which is why accuracy is not used as the selection metric.}
