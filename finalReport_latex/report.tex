...
Smart Workout is a Retrieval-Augmented Generation (RAG)-assisted Decision Support System and Business Intelligence dashboard for personalized weight-training planning. The project addresses the limitation of generic workout routines by combining gym membership and check-in records, exercise tracking data, lifestyle and sleep indicators, nutrition context, and an exercise knowledge base into one descriptive, predictive, and prescriptive analytics workflow.

The system was implemented as a full-stack web application using a React frontend and a FastAPI backend. Raw data was transformed through feature engineering into utilization and time-pattern signals, membership and estimated revenue signals, lifestyle readiness indicators, workout-intensity features, calorie-burn-rate targets, final training-score variables, and RAG retrieval inputs. These engineered features support two decision contexts: executive users who need branch, segment, cohort, and demand insights, and individual users who need readiness-aware training guidance.

The predictive layer evaluates two supervised machine learning tasks: calorie-burn-rate regression and three-class workout-intensity classification. XGBoost was selected for calorie-burn-rate estimation with RMSE 15.820 kcal/hr and $R^2=0.970$. XGBoost was also selected for intensity classification using macro-F1 as the main metric because the Low, Mid, and High classes are imbalanced. The classifier's modest accuracy is reported transparently as a hard three-class task with label-leakage prevention, corrected train/inference feature alignment, and class-balancing improvements. The model audit also documents fixes for gender feature mismatch, balanced class weighting, stable model serialization, and honest regression reporting.

The prescriptive layer combines model outputs with readiness rules and RAG-retrieved exercise evidence. It generates personalized plan recommendations, plan rationale, safety notes, and assistant responses grounded in exercise instructions, target body part, equipment, predicted intensity, and readiness band. The final dashboard integrates executive summaries, membership utilization and revenue signals, lifestyle segment analysis, client readiness inputs, exercise plan analytics, nutrition context, Tableau map embedding, and RAG-assisted explanation. The result is a decision-support prototype that links data preparation, model evaluation, explainable recommendations, and dashboard-based action decisions in one workflow.
\clearpage
...
XGBoost Regressor & 15.820 & 10.424 & 0.970 & Yes \\
\bottomrule
\end{tabular}
\end{table}

The XGBoost Regressor is selected because it produces the strongest calorie-burn-rate estimation result, with RMSE 15.820 kcal/hr and $R^2=0.970$. This score is intentionally reported as an honest deployment-aware result. The earlier inflated regression performance was corrected by removing features that created misleading training-time advantage or inference mismatch. In particular, gender was excluded because the deployed user form does not collect it.
...
Classifier imbalance & Mid class was the majority class, so the model under-predicted Low and High intensity. & Balanced class weighting improved macro-F1 from 0.304 to 0.342, Low recall to 21.5\%, and High recall to 30\%. \\
...
Inflated regression score & Some training features made the regression score appear too optimistic under deployment conditions. & The honest cross-validated result is $R^2=0.970$ and RMSE 15.82 kcal/hr, with XGBoost selected over Random Forest. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{center}
\Large\bfseries APPENDIX B 

\vspace{0.8cm}

\Large\bfseries Exploratory Data Analysis Figures
\end{center}
