...
const defaultProfile: UserProfileRequest = {
  age: 22,
  height_cm: 165,
  weight_kg: 60,
  sleep_hours: 7,
...
export default function ProfileForm({ loading, onSubmit }: ProfileFormProps) {
  const [profile, setProfile] = useState<UserProfileRequest>(defaultProfile)
  const [errors, setErrors] = useState<Record<string, string>>({})

  const setField = <K extends keyof UserProfileRequest>(key: K, value: UserProfileRequest[K]) => {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (profile.age < 13 || profile.age > 90) nextErrors.age = 'Age must be between 13 and 90.'
    if (profile.height_cm < 100 || profile.height_cm > 230) nextErrors.height_cm = 'Height must be between 100 and 230 cm.'
    if (profile.weight_kg < 30 || profile.weight_kg > 250) nextErrors.weight_kg = 'Weight must be between 30 and 250 kg.'
    if (profile.sleep_hours < 0 || profile.sleep_hours > 14) nextErrors.sleep_hours = 'Sleep must be between 0 and 14 hours.'
    if (profile.stress_level < 1 || profile.stress_level > 10) nextErrors.stress_level = 'Stress level must be 1 to 10.'
    if (!/^\d{2,3}\/\d{2,3}$/.test(profile.blood_pressure)) nextErrors.blood_pressure = 'Use blood pressure format like 120/80.'
    if (profile.resting_heart_rate < 35 || profile.resting_heart_rate > 140) nextErrors.resting_heart_rate = 'Heart rate must be between 35 and 140 bpm.'
    if (profile.sessions_per_week < 1 || profile.sessions_per_week > 6) nextErrors.sessions_per_week = 'Sessions per week must be 1 to 6.'
      ...
      <section className="panel">
        ...
        <div className="form-grid">
          <div className="field">
            ...
            <input id="age" type="number" min={13} max={90} value={profile.age} onChange={(event) => setField('age', Number(event.target.value))} />
            {errors.age && <span className="field-error">{errors.age}</span>}
          </div>
          <div className="field">
            <label htmlFor="height">Height (cm)</label>
            <input id="height" type="number" min={100} max={230} value={profile.height_cm} onChange={(event) => setField('height_cm', Number(event.target.value))} />
            {errors.height_cm && <span className="field-error">{errors.height_cm}</span>}
          </div>
          <div className="field">
            <label htmlFor="weight">Weight (kg)</label>
            <input id="weight" type="number" min={30} max={250} value={profile.weight_kg} onChange={(event) => setField('weight_kg', Number(event.target.value))} />
      ...
      <section className="panel">
        ...
        <div className="form-grid">
          <div className="field">
            ...
            <input id="sleep" type="number" min={0} max={14} step={0.5} value={profile.sleep_hours} onChange={(event) => setField('sleep_hours', Number(event.target.value))} />
            {errors.sleep_hours && <span className="field-error">{errors.sleep_hours}</span>}
          </div>
          <div className="field">
            <label htmlFor="stress">Stress level</label>
            <input id="stress" type="number" min={1} max={10} value={profile.stress_level} onChange={(event) => setField('stress_level', Number(event.target.value))} />
          ...
          <div className="field">
            ...
            <input id="hr" type="number" min={35} max={140} value={profile.resting_heart_rate} onChange={(event) => setField('resting_heart_rate', Number(event.target.value))} />
      ...
      <section className="panel">
        ...
        <div className="form-grid">
          ...
          <div className="field">
            ...
            <input id="sessions" type="number" min={1} max={6} value={profile.sessions_per_week} onChange={(event) => setField('sessions_per_week', Number(event.target.value))} />
